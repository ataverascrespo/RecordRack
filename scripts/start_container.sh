...
docker run -d -p 5184:5184 --name recordrack recordrack_image -e \
    $ConnectionStrings $AppSettings $CloudinarySettings_ApiKey $CloudinarySettings_ApiSecret \
    $CloudinarySettings_CloudName $SendgridSettings_ApiKey $SendgridSettings_FromEmail \ 
    $SpotifySettings_ClientID $SpotifySettings_ClientSecret $SqidsSettings_Alphabet
